...
namespace Tcdev.Dsm.Matrix
{
    ...
    /// </remarks>
    class PartitionerMarkII
    {
        SquareMatrix _sm;
        
        #region Stuff for debugging
        /*
         * DEBUG variables
         */
        ...
        public PartitionerMarkII(SquareMatrix matrix)
        {
            _sm = matrix;
        }
                                    ...
                                    if (x != y)  // ignore the diagonal
                                    {
                                        if (TrueMatrixValue(vector, x, y) == 0)
                                        {
                                            ...
                                            if ( !permMap.ContainsKey(p) )
                                            {
                                                ...
                                                newScore = Score(vector);

                                                if (newScore > currentScore)
                                                {
                                                    _accepted++; // for trace info
                                                    currentScore = newScore;
                                                    doLoop = true; // increasing score so continue
                                                }
                                                else
                                                {
                                                    _rejected++; // for trace info
